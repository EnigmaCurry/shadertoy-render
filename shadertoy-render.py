...
if os.name == "nt":
    os.environ[
        "GLFW_LIBRARY"
    ] = r"C:\glfw-3.3.2.bin.WIN64\glfw-3.3.2.bin.WIN64\lib-vc2019\glfw3.dll"
...
class RenderingCanvas(app.Canvas):
    def __init__(
        ...
        self.set_Buf_uniform("iMouse", (0.0, 0.0, 0.0, 0.0))
        ...
        self.set_Buf_uniform("iOffset", (0.0, 0.0))
        self.set_Buf_uniform(
            "iResolution", (self.physical_size[0], self.physical_size[1], 0.0)
        )
        ...
        if interactive:
            ...
            self._fbo = gloo.FrameBuffer(
                self._rendertex, gloo.RenderBuffer(shape=render_size[::-1])
            )

            # Allocate buffer to hold final image

            self._img = numpy.zeros(
                shape=self._output_size[::-1] + (4,), dtype=numpy.uint8
            )
            ...
            self.program["iResolution"] = self._output_size + (0.0,)
            self.ensure_timer()
    ...
    def set_Buf_channel_input(self):
        for i in range(max_iChannels):
            for j in range(max_iChannels):
                self._BufX[i]["iChannel%d" % j] = self._texX[
                    self._doubleFboid - 1 if j >= i else self._doubleFboid
                ][j]
                self._BufX[i]["u_channel%d" % j] = self._texX[
                    self._doubleFboid - 1 if j >= i else self._doubleFboid
                ][j]
                self._BufX[i]["iChannelResolution[%d]" % j] = self._output_size + (0.0,)
    ...
    def init_BufX(self):
        ...
        for i in range(max_iChannels):
            ...
            self._fboX[0].append(
                gloo.FrameBuffer(
                    self._texX[0][i], gloo.RenderBuffer(shape=self._render_size[::-1])
                )
            )
            self._texX[1].append(
                gloo.Texture2D(
            ...
            self._fboX[1].append(
                gloo.FrameBuffer(
                    self._texX[1][i], gloo.RenderBuffer(shape=self._render_size[::-1])
                )
            )
            self._BufX.append(gloo.Program(vertex, fragment_template % error_shader))
        ...
        if not self._paused:
            if self._interval == "auto":
                self.program["iGlobalTime"] = (
                    time.perf_counter() - self._clock_time_zero
                )
                for i in range(max_iChannels):
    ...
    def draw(self):
        ...
        if self._interactive:
            ...
            if (
                self._render_frame_count is not None
                and self._render_frame_index >= self._render_frame_count
            ):
                app.quit()
                return
            ...
            self.program["iMouse"] = (
                self.program["iMouse"][0],
                self.program["iMouse"][1],
                self.program["iMouse"][2],
                -abs(self.program["iMouse"][3]),
            )
            self.set_channel_input()
            self.set_Buf_channel_input()
        else:
    ...
    def on_timer(self, event):
        if self._interactive:
            ...
            rendered_tile_count = (
                self._tile_index + self._render_frame_index * self._tile_count
            )
            ...
            print(
                "Tile %d / %d (%.2f%%); %s elapsed; %s remaining; %s total"
                % (
                    rendered_tile_count,
                    total_tile_count,
    ...
    def ensure_timer(self):
        if not self._timer:
            self._timer = app.Timer(
                ("auto" if self._ffmpeg_pipe else self._interval),
    ...
    def update_timer_state(self):
        if not self._paused:
            self._clock_time_zero = time.perf_counter() - self.program["iGlobalTime"]
            self.ensure_timer()
        else:
            if self._profile:
                self.ensure_timer()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bitrate", type=str, default="5M", help="bitrate of video 1M 2M 15M etc"
    )
    parser.add_argument(
        "--pos", type=str, help="Position of the viewport, e.g. 100,100 (string)."
    )
    ...
    position = (
        tuple(int(i) for i in args.pos.split(",")) if args.pos is not None else None
    )
        ...
        if file_ext == ".mov":
            ffmpeg = subprocess.Popen(
                (
                    ffmpeg_file,
                    "-loglevel",
                    ("verbose" if args.verbose else "panic"),
                    "-r",
                    "%d" % args.rate,
                    "-f",
                    "rawvideo",
            ...
            if file_ext == ".webm":
                ...
                ffmpeg = subprocess.Popen(
                    (
                        ...
                        "slow",
                        "-c:v",
                        "libx264",  # 'libx264', h264_nvenc
                        "-y",
                        args.output,
                    ),
                    stdin=subprocess.PIPE,
                )
    ffmpeg_pipe = ffmpeg.stdin if ffmpeg is not None else None
